...
const PlanSelector = ({ onClose }) => {
  ...
  const startCheckout = async (trialDays = 14) => {
    ...
    try {
      ...
      setError(e.response?.data?.error || e.message || 'Checkout error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="plan-selector__backdrop">
      <div className="plan-selector">
        <h3>Select a plan — trial or subscribe immediately</h3>
        ...
        <div className="plan-selector__actions">
          <button className="plan-selector__confirm" onClick={() => startCheckout(14)} disabled={loading}>{loading ? 'Redirecting…' : 'Start 14‑day trial'}</button>
          <button className="plan-selector__confirm plan-selector__buy-now" onClick={() => startCheckout(0)} disabled={loading}>{loading ? 'Redirecting…' : 'Subscribe now'}</button>
          <button className="plan-selector__cancel" onClick={onClose}>Cancel</button>
        </div>
      </div>
    </div>
  );
